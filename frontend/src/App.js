...
`;
  // ***************************************************************
  // Keep these in state
  // Whether Modal visible or not
  const [preview, setPreview] = useState(false);
  // The email template
  const [template, setTemplate] = useState(defaultTemplate);
  // The email with tags replaced
  ...
  // Replace all tags found in 'template' by those identified by 'keyValues'
  ...
  let processTemplate = () => {
    let keyValues = { contact_first_name: recipient && recipient.name ? recipient.name.split(' ')[0] : '',
  ...
  // *****************************************
  // Handlers for the Replacement tags' inputs
  ...
  // From the Modal, we call this to send the email and exit the Modal
